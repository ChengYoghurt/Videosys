...
class EvolutionSearcher(object):
    ...
    def __init__(self, opt, engine, full_timesteps, time_step, ref_videos, ref_sigma, device, dpm_params=None):
        self.opt = opt
        self.engine = engine
    ...
    #     prompt_idx = random.randint(0, len(self.ref_videos) - 1)
    #     cand_latent = self.generate_cand_video(cand=cand, prompt_idx=prompt_idx)
    #     # MSE Calculation
    #     ref_latent = self.ref_videos[prompt_idx]
        ...
        if self.use_ddim_init_x is False:
            ...
            if self.opt.dpm_solver:
                ...
                init_x = make_ddim_timesteps(ddim_discr_method=self.ddim_discretize, num_ddim_timesteps=self.time_step,
                                                        num_ddpm_timesteps=self.sampler.num_timesteps, verbose=False) # not used
